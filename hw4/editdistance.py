...
class EditDistance:
    def __init__(self, s, t, delta=lambda x, y: 1 if x != y else 0):
        self.s = s
        self.t = t
        self.delta = delta
        self.n = len(s)
    ...
    def calc(self):
        ...
        for i in range(self.n + 1):
            self.V[i, 0] = self.V[i - 1, 0] + self.delta("", "_")
            if i != 0:
                self.G.add_edge(self.getNodeIDfromIndex(i, 0), self.getUpNodeId(i, 0))
        for j in range(self.m + 1):
            self.V[0, j] = self.V[0, j - 1] + self.delta("_", "")
            if j != 0:
        ...
        for i in range(1, self.n + 1):
            ...
            for j in range(1, self.m + 1):
                ...
                up = self.V[i - 1, j] + self.delta("", "_")
                left = self.V[i, j - 1] + self.delta("_", "")
                min_value = min(diag, up, left)
                self.V[i, j] = min_value
                if diag == min_value:
...
def main():
    print("Running Edit-Distance Algorithm")
    ed = EditDistance("adsdf", "sdjfhsdf", lambda x, y: 2)
    print(ed.V)
    print(ed.delta(3, 6))

    ed2 = EditDistance("CCTGCTGCAG", "GATGTGCCG")
    print(ed2.V)
    print(ed2.delta(1, 2))
    ed2.calc()
    print(ed2.V)

    print(list(ed2.G.nodes))
    print(list(ed2.G.edges))

    for path in nx.all_simple_paths(ed2.G, 63, 0):
        print(path)

    ed2.getAllAlginments()
